...
export async function guardarIdea(idea: Idea): Promise<void> {
  const { error } = await supabase.from('ideas').insert({
    ...
    problema_resuelve: idea.problemaResuelve || null,
    beneficios_esperados: idea.beneficiosEsperados || null,
...
export async function leerIdeas(): Promise<Idea[]> {
  ...
  return (data || []).map(row => ({
    ...
    problemaResuelve: row.problema_resuelve || undefined,
    beneficiosEsperados: row.beneficios_esperados || undefined,
