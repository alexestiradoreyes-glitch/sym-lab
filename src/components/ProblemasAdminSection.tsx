...
export default function ProblemasAdminSection({ problemas }: Props) {
  ...
  const totalPaginas = Math.max(1, Math.ceil(problemasVivos.length / PROBLEMAS_POR_PAGINA))
  const paginaActual = Math.min(pagina, totalPaginas)
  const pagElem      = problemasVivos.slice((paginaActual - 1) * PROBLEMAS_POR_PAGINA, paginaActual * PROBLEMAS_POR_PAGINA)
